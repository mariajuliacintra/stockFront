...
const ImportModal = ({ open, onClose, data, onAlert, onSuccess }) => {
    ...
    // Headers padrão e de controle que DEVEM SER IGNORADOS no mapeamento dinâmico
    const standardHeaders = [
        'name', 'brand', 'sapCode', 'description', 'minimumStock', 
        'quantity', 'expirationDate', 'fkIdLocation', 'fkIdCategory',
        // CHAVES DE CONTROLE INTERNO (PROTEÇÕES)
        'isEdited', 'isMissingCategory', 'tempSelectedSpecId', 'tempSpecValue', 
        'itemSpecs', 
        'aliases' 
    ];
    
    // Mapeamento de SpecKeys para o ID (usado no handleSubmit)
    ...
    // Filtra as chaves de specs dinâmicas
    const getDynamicSpecKeys = () => {
        if (!editableRows || editableRows.length === 0) return [];
        
        const firstRowKeys = Object.keys(editableRows[0]);
        return firstRowKeys.filter(key => !standardHeaders.includes(key));
    };
        ...
        <Modal open={open} onClose={onClose} sx={modalStyles.modalOverlay}>
            <Box sx={modalStyles.modalContent}>
                    ...
                    <Box sx={{ mt: 1, maxHeight: 'calc(90vh - 250px)', overflowY: 'auto', width: '100%' }}>
                        <TableContainer component={Paper}>
                            <Table size="small" stickyHeader>
                                <TableHead>
                                    <TableRow>
                                        {/* REMOVIDO: Conteúdo da primeira célula que não era útil */}
                                        ...
                                        {/* Cabeçalhos de Especificação Dinâmica (DO EXCEL) */}
                                        {dynamicSpecKeys.map(key => (
                                            <TableCell key={key} sx={modalStyles.tableHeader}>
                                                {key} 
                                                <IconButton 
                                                    size="small" 
                                                    onClick={() => handleRemoveSpec(index, key)} 
                                                    sx={{ ml: 0.5, color: 'red' }}
                                                >
                                ...
                                <TableBody>
                                    {editableRows.map((row, index) => (
                                        <TableRow key={index} sx={row.isMissingCategory || Number(row.quantity) <= 0 ? modalStyles.pendingRow : row.isEdited ? modalStyles.editedRow : {}}>
                                            <TableCell>
                                                {/* ÍCONE DE DELETE DA LINHA (MANTIDO) */}
                                            ...
                                            {/* Células de Especificação Dinâmica (VALORES DO EXCEL) */}
                                            {dynamicSpecKeys.map(key => (
                                                <TableCell key={key}>
                                                     <TextField 
                                                         value={row[key] || ''}
                                            ...
                                            <TableCell sx={{ display: 'flex', alignItems: 'center', p: '4px 4px 4px 0'}}>
                                                ...
                                                <TextField 
                                                    ...
                                                    placeholder="Valor"
                                                    sx={{ width: 80, mr: 0.5 }}
                                                    disabled={!row.tempSelectedSpecId} // Desabilita se a Spec não foi selecionada
                                                />
                ...
                <Button
                    ...
                    // A desativação agora considera a quantidade > 0

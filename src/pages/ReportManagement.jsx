...
const REPORTS = [
  {
    ...
    description:
      "Relatório completo de todos os itens, localizações e especificações.",
    type: "general",
    icon: ReportIcon,
  },
  {
  ...
  },
  {
    ...
    description:
      "Histórico detalhado das últimas movimentações de entrada e saída.",
    type: "transactions",
  ...
  const handleDownload = async (reportType, format, reportTitle) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 404) {
        handleAlert(
          "Erro: Endpoint não encontrado (404). Verifique as rotas da API.",
          "error"
        );
      } else {
  ...
  const handleImport = async (event) => {
    ...
    try {
      ...
      if (validRows.length > 0 || invalidRows.length > 0) {
        ...
        handleAlert(
          "Pré-visualização carregada. Edite as categorias e finalize.",
          "info"
        );
      } else {
        handleAlert(
          "O arquivo Excel não contém dados de itens válidos para processamento.",
          "warning"
        );
      }
    } catch (error) {
    ...
    <Box sx={styles.pageLayout}>
      ...
      <Container component="main" sx={styles.container}>
        ...
        <Box sx={styles.cardContainer}>
          <Box
            sx={{ ...styles.header, justifyContent: "space-between", mb: 3 }}
          >
            ...
            {REPORTS.map((report) => (
              <Grid item xs={12} key={report.title}>
                <Paper
                  ...
                  aria-busy={Boolean(
                    downloading && downloading.startsWith(report.type)
                  )}
                >
                  ...
                  <Box sx={styles.actionButtons}>
                    <Button
                      ...
                      startIcon={<ExcelIcon />}
                      onClick={() =>
                        handleDownload(report.type, "excel", report.title)
                      }
                    ...
                    <Button
                      ...
                      startIcon={<PdfIcon />}
                      onClick={() =>
                        handleDownload(report.type, "pdf", report.title)
                      }
                      sx={styles.pdfButton}
                      disabled={Boolean(
                        downloading && downloading.startsWith(report.type)

...
import sheets from "../services/axios"; // Serviço de API (sheets/axios)
...
  // ✅ NOVO: Função para buscar o perfil usando o ID da API
  const fetchUserProfile = useCallback(async (id) => {
    setLoading(true);
    try {
      // Usa sheets.getUserProfile com o ID para buscar Nome e Email
...
        // Atualiza o estado do formulário com dados APENAS da API
        setForm({ nome: userData.name, email: userData.email });
      } else {
        showSnackbar("Falha ao carregar os dados do perfil.", "error");
        navigate('/perfil');
      }
    } catch (e) {
      console.error("Erro na requisição da API:", e);
      showSnackbar("Erro ao buscar dados do perfil. Tente novamente.", "error");
      navigate('/perfil');
    } finally {
      setLoading(false);
    }
...
    
    // ✅ CORREÇÃO: Apenas lê o idUsuario. Não busca mais userName/userEmail.
    const idUsuario = localStorage.getItem("idUsuario");
    
    if (idUsuario) {
      setUserId(idUsuario);
      // CHAMA A API para buscar Nome e Email
      fetchUserProfile(idUsuario);
    } else {
      // Se não encontrar o ID, redireciona para o login
      showSnackbar("Sessão expirada. Faça login novamente.", "warning");
      navigate("/login");
    }

    // Lógica do refresh token (mantida, mas removemos a linha onde você tentou adicionar a remoção do refresh_token)
...
  // ✅ CORREÇÃO: Função que atualiza APENAS o estado local e redireciona.
  const updateLocalUserAndState = (updatedData) => {
    // Não toca no localStorage para Nome/Email
...
    try {
      // Usa o userId (idUsuario) para a chamada PUT
...
    setLoading(true);
    try {
      // Usa o userId (idUsuario) para a chamada DELETE
...
      // ✅ Não há mais userName e userEmail para remover!
      navigate("/");
    } catch (error) {
      console.error("Erro ao deletar o perfil:", error);
      showSnackbar(
        error.response?.data?.error || "Erro ao deletar o perfil.",
        "error"
      );
    } finally {
      setLoading(false);
...
      // Assumindo que a verificação de email não precisa do ID na payload, apenas do novo email e código.
